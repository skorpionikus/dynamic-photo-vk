...
class Target:
    def __init__(self, p_cookie, l_cookie, ua, remixttpid=''):
        ...
        self.re_share = compile(r'"upload":{"url":"(.+)"},"edited"')
        self.re_pe_hashes = compile(r'"pe_hash":"(\w+)"')
        self.re_ids = compile(r'"id":"(\w+)"')
    ...
    def get_hash(self, pid):
        ...
        ids = findall(self.re_ids, r.text)
        return [n for i,n in enumerate(findall(self.re_pe_hashes, r.text)) if ids[i] == photo_id][0]
    ...
    def change_photo(self, path, pid):
        ...
        url = search(self.re_share, res.text).group(1).replace('\\','')
